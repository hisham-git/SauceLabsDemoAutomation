...
public class PDFGenerator {
	...
	public void comonStructure(PdfPTable t, List<String> dataList, BaseColor b) {

		PdfPTable inner = new PdfPTable(1);
		inner.getDefaultCell().setFixedHeight(65);
		for (String data : dataList) {
			inner.addCell(data);
		}
		
		PdfPCell dataCell = new PdfPCell(inner);
		t.addCell(dataCell);
	}
...
//		this.comonStructure(t, "Notes", BaseColor.LIGHT_GRAY);
	...
	public void generatePDF(PdfPTable t, Paragraph note, String pdfName, ListMultimap<String, List<String>> stepLog, String host, String exedate, String desc, String suiteName,
			String methodStatus) throws DocumentException, MalformedURLException, IOException {
		int step = 1;
		...
		if (!stepLog.isEmpty()) {
			for(String stepMsg: stepLog.keySet()){
				this.comonStructure(t, "#" + String.valueOf(step), BaseColor.WHITE);
				this.comonStructure(t, stepMsg, BaseColor.WHITE);
				List<List<String>> msgLists = stepLog.get(stepMsg);
				Log.logDebugMessage("Number of Message lists: " + msgLists.size());
				for (List<String> msgList : msgLists) {
					this.comonStructure(t, msgList, BaseColor.WHITE);
				}
				Log.logDebugMessage("Step failed: "+msgLists.get(2).contains("Failed"));
				if(msgLists.get(2).contains("Failed")) {
